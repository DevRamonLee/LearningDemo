<template>
  <div>
    <h2> 在动态组件上使用 keep-alive </h2>
    <p> 使用 keep-alive  可以让组件在第一次被创建的时候缓存下来 </p>
    <button
      v-for="tab in tabs"
      v-bind:key="tab"
    ...
    <h2> 异步组件 </h2>
    <p> 如果一开始就加载所有的组件，是比较耗时的，我们可以将一些组件定义为异步组件，在需要的时候进行加载（异步组件第一次加载完会被缓存下来）</p>
    <p> 点击按钮后，第一个延迟 3000 毫秒，从服务器加载,第二个不延迟从服务器加载</p>
...
const later = Vue.component('later', function (resolve) {
    setTimeout(function () {
        require(['../async/later1.vue'], resolve)
    }, 3000);
});
const later2 = Vue.component('later2', function (resolve) {
    require(['../async/later2.vue'], resolve)
});
